...
// =========================================================================
// CẤU HÌNH CÁC ĐƯỜNG LINK GOOGLE SHEET CỦA BẠN Ở ĐÂY
// =========================================================================
// 1. File lưu danh sách Hội Viên Tổng
...
// 2. File lưu Kết Quả Thi (Cấp & Đẳng)
const SHEETDB_THICAP_URL = "https://script.google.com/macros/s/AKfycbwMA4jQWOdplfR2sQmR40HqnWrcGNh6syxbyxaOvgNf_h0t_qV57zk8N23cAl6qQkXB/exec"; 

// 3. File lưu riêng Doanh Thu (Học phí, Lệ phí thi)
const SHEETDB_DOANHTHU_URL = "https://script.google.com/macros/s/AKfycbw5urwKyih7-tqOFbUNuIuI9q2fqsskOzC_XlPS9e_ZSs_mTLIs3h67UnUpO-jKJwZr/exec"; 
// =========================================================================
...
const App = () => {
  const [user, setUser] = useState(() => {
    const saved = localStorage.getItem('vtf_current_user');
    return saved ? JSON.parse(saved) : null;
  });
  const [isLoggedIn, setIsLoggedIn] = useState(() => !!localStorage.getItem('vtf_current_user'));
  ...
  // ==========================================
  // STATE MỚI CHO TÌM KIẾM ĐIỂM DANH VÀ HỌC PHÍ LỚP
  const [searchDiemDanh, setSearchDiemDanh] = useState('');
  const [searchFeeClassStudent, setSearchFeeClassStudent] = useState('');
  // ==========================================
  ...
  const [archivedExams, setArchivedExams] = useState(() => JSON.parse(localStorage.getItem('vtf_archived_exams') || '[]'));
  ...
  useEffect(() => {
    const syncData = () => {
      ...
      const savedArchivedExams = localStorage.getItem('vtf_archived_exams'); if (savedArchivedExams) setArchivedExams(JSON.parse(savedArchivedExams));
  ...
  const syncExamToGoogleSheet = async () => {
    ...
    try {
      ...
      if (revenueData.length > 0) {
        await axios.post(SHEETDB_DOANHTHU_URL, JSON.stringify({ data: revenueData }), { headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
      }
      ...
      const updatedArchived = [selectedExam, ...archivedExams];
      setArchivedExams(updatedArchived);
      localStorage.setItem('vtf_archived_exams', JSON.stringify(updatedArchived));

      const remainingExams = exams.filter(e => e.id !== selectedExam.id);
      setExams(remainingExams);
      localStorage.setItem('vtf_exams', JSON.stringify(remainingExams));
      
      const updatedExamCandidates = { ...examCandidates };
      delete updatedExamCandidates[selectedExam.code];
      setExamCandidates(updatedExamCandidates);
      localStorage.setItem('vtf_exam_candidates', JSON.stringify(updatedExamCandidates));

      setSelectedExam(null);
      fetchData(user);
      message.success("Đã hoàn tất lưu Kết Quả và Doanh Thu vào các Sheet tương ứng!");
  ...
  const handleAdminChotSo = async () => {
    ...
    setLoading(true);
    try { 
      await axios.post(SHEETDB_DOANHTHU_URL, JSON.stringify({ data: dataToSync }), { headers: { 'Content-Type': 'text/plain;charset=utf-8' } }); 
      ...
      message.success("Đã chốt sổ Tổng doanh thu và đẩy lên Sheet Doanh Thu thành công!"); 
  ...
  const handleLogin = (values) => {
    ...
    if (found) { 
        setUser(found); 
        setIsLoggedIn(true); 
        localStorage.setItem('vtf_current_user', JSON.stringify(found));
        fetchData(found); 
    } 
      ...
      <Layout>
        <Header style={{ background: '#fff', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '0 20px', borderBottom: '1px solid #eee' }}>
          <Space>
            ...
            <Button onClick={() => { localStorage.removeItem('vtf_current_user'); window.location.reload(); }} danger>Đăng xuất</Button>
          </Space>
        </Header>
          ...
          {activeMenu === 'dashboard' ? (
            ...
            <div style={{ background: '#fff', padding: '20px', borderRadius: 8, minHeight:'80vh' }}>
              {!selectedClass ? (
                ...
                <>
                  <Button icon={<ArrowLeftOutlined />} onClick={() => {setSelectedClass(null); setSelectedStudentKeys([]); setSearchDiemDanh('');}} style={{ marginBottom: 15 }}>Quay lại</Button>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 15, alignItems: 'center' }}>
                    <Title level={4} style={{margin:0}}>Lớp: {selectedClass.name}</Title>
                    <Space>
                      <Input.Search placeholder="Tìm tên/mã võ sinh..." allowClear onChange={e => setSearchDiemDanh(e.target.value)} style={{width: 200}} />
                  ...
                  <Table 
                    ...
                    dataSource={displayClassStudents.filter(s => (s.hoten || '').toLowerCase().includes(searchDiemDanh.toLowerCase()) || (s.mahv || '').toLowerCase().includes(searchDiemDanh.toLowerCase()))} 
                    bordered pagination={false}
                    columns={[
                      { title: 'STT', key: 'stt', width: 60, render: (t,r,i) => i+1 },
                      { title: 'Sửa', width: 60, render: (t, r) => <EditOutlined style={{color:'#1d1ab6', cursor:'pointer'}} onClick={() => { 
                ...
                <Tabs items={[
                  { key: '1', label: 'Quản lý Học Phí Lớp', children: (
                    !selectedFeeClass ? (
                      ...
                      <>
                        <Button icon={<ArrowLeftOutlined />} onClick={() => { setSelectedFeeClass(null); setSearchFeeClassStudent(''); }} style={{ marginBottom: 15 }}>Quay lại</Button>
                        <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: 15, alignItems: 'center'}}>
                            <Title level={4} style={{margin: 0}}>Chi tiết học phí Lớp: {selectedFeeClass.name}</Title>
                            <Input.Search placeholder="Tìm tên/mã võ sinh..." allowClear onChange={e => setSearchFeeClassStudent(e.target.value)} style={{width: 250}} />
                        </div>
                        <Table 
                          dataSource={(classStudents[selectedFeeClass.id] || []).filter(s => s.trangthai === 'Hoạt động' && ((s.hoten || '').toLowerCase().includes(searchFeeClassStudent.toLowerCase()) || (s.mahv || '').toLowerCase().includes(searchFeeClassStudent.toLowerCase())))} 
                          rowKey="key" bordered
                          columns={[
            ...
            <div style={{ background: '#fff', padding: '20px', borderRadius: 8, minHeight:'80vh' }}>
              ...
              <Tabs items={[
                ...
                { key: '3', label: 'Danh sách Mã Kỳ Thi đã lưu', children: (
                  <Table dataSource={archivedExams} rowKey="id" bordered locale={{ emptyText: "Chưa có mã kỳ thi nào trong kho lưu trữ." }}
                    columns={[
                      ...
                      { title: 'Thao tác', width: 200, render: (t, r) => (
                        <Space>
                          ...
                          {isAdmin && (
                            <Popconfirm title="Xóa vĩnh viễn kỳ thi này khỏi kho lưu trữ?" onConfirm={() => {
                              const updatedArchived = archivedExams.filter(e => e.id !== r.id);
                              setArchivedExams(updatedArchived);
                              localStorage.setItem('vtf_archived_exams', JSON.stringify(updatedArchived));
                              message.success("Đã xóa vĩnh viễn kỳ thi!");
                            }}>
